...
output += `// Cores whose bare name is too generic to be treated as a valid skill on its\n`;
output += `// own (must only match via their specific detail skills).\n`;
output += `const SELF_INDEX_EXCLUDED_CORES = new Set(["RPA"]);\n\n`;
...
output += `    }\n`;
output += `    // Also map the core skill name itself — EXCEPT for cores where the bare\n`;
output += `    // core name is too generic to be a real skill (e.g. "RPA").\n`;
output += `    if (!SELF_INDEX_EXCLUDED_CORES.has(core)) {\n`;
